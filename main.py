...
class Ambulance(db.Model):
    ambulance = db.StringProperty(required=True)
    marked = db.StringProperty(required=True)


def fill_ambulance_database():
    all_ambulance = db.GqlQuery("select * from Ambulance")
    for i in all_ambulance:
        i.delete()
    registered_json = """
    ...
    for i in list:
        name = str(i["name"])
        mark = "0"
        x = Ambulance(ambulance=name, marked=mark)
        x.put()


fill_ambulance_database()
        ...
        if not found:
            ...
            if total_logged_in > 0:
                ...
                if current_bucket == total_logged_in:
                    current_bucket = 0
                logging.error(
                    "total logged in " + str(total_logged_in) + '\n' + "current bucket " + str(current_bucket))
...
class CompletedHandler(Handler):
    def get(self):
        ...
        vendor_match = False
        for i in all_ambulance:
            if i.ambulance == vendor:
                vendor_match = True
                break
        if not vendor_match:
            self.response.out.write("vendor name does not match")
        else:
            all_requests = db.GqlQuery("select * from RequestNew")
            found = False
            for i in all_requests:
                if i.phone == phone:
                    request = CompletedRequest(type=i.type,
                                               injured=i.injured,
                                               latitude=i.latitude,
                    ...
                    request_id = str(i.key().id())
                    all_maps = db.GqlQuery("select * from MappingRequests")
                    for m in all_maps:
                    ...
                    i.delete()
                    request.put()
                    link = "http://backend-108.appspot.com/markasnotbusy?name=" + vendor
                    notbusyrequest = requests.get(link)
                    self.response.out.write(notbusyrequest.text)
...
class RegisteredPlacesComputationHandler(Handler):
    def get(self):
        ...
        list = x["places"]
        output_list = []
        all_ambulances = db.GqlQuery("select * from Ambulance")
        for i in list:
            inner_dict = {}
            des_lat = i["lat"]
            des_lng = i["lon"]
            ...
            ambulance_name = i["name"]
            for i in all_ambulances:
                if i.ambulance == ambulance_name:
                    inner_dict["marked"] = i.marked
                    break
...
class ServiceCompleteHandler(Handler):
    ...
    def post(self):
        ...
        if password == vendor_pass:
            self.redirect("/requestcomplete?phone=" + phone + "&vendor=" + vendor)
        else:
...
class PersonPendingHandler(Handler):
    def get(self):
        ...
        if check_valid_cookie(user_id) == True:
            ...
            person = db.get(key)
            name = person.name
            self.render("person.html", list_of_marked=list_of_marked, list_of_results=list_of_results, name=name,
                        list_of_mapping_id=list_of_mapping_id)
        else:
            self.redirect('/login')
...
class AllCompletedRequestsHandler(Handler):
    def get(self):
        all_data = db.GqlQuery("select * from CompletedRequest order by date desc")
    ...
    def get(self):
        ...
        for i in all_ambulances:
            if i.ambulance == name:
                i.delete()
                found = True
                x = Ambulance(ambulance=name, marked="1")
                x.put()
                break
        if not found:
            self.response.out.write("NAME NOT FOUND")
        else:
            self.response.out.write("DONE!!")


class MarkNotBusyHandler(Handler):
    def get(self):
        name = self.request.get("name")
        all_ambulances = db.GqlQuery("select * from Ambulance")
        found = False
        for i in all_ambulances:
            if i.ambulance == name:
                i.delete()
                found = True
                x = Ambulance(ambulance=name, marked="0")
                x.put()
                break
        if not found:
            self.response.out.write("NAME NOT FOUND")
...
app = webapp2.WSGIApplication([
    ...
    ('/sos', SosHandler),
    ('/markasbusy', MarkBusyHandler),
    ('/markasnotbusy', MarkNotBusyHandler)
], debug=True)
